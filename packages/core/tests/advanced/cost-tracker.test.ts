...
describe('CostTracker', () => {
  ...
  describe('track operation', () => {
    it('should track OpenAI operation', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-4', 1000, 500);
    ...
    it('should calculate correct cost for OpenAI GPT-3.5', () => {
      ...
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-3.5-turbo', 1000, 1000);
    ...
    it('should track Anthropic Claude operations', () => {
      const operation = tracker.trackOperation('op-2', 'anthropic', 'claude-3-sonnet', 2000, 1000);
    ...
    it('should track Google Gemini operations', () => {
      const operation = tracker.trackOperation('op-3', 'google', 'gemini-pro', 1000, 500);
      expect(operation.provider).toBe('google');
      expect(operation.model).toBe('gemini-pro');
    });

    it('should handle zero tokens', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-4', 0, 0);
  ...
  describe('cost calculation precision', () => {
    it('should handle fractional token counts', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-4', 1234, 5678);
    ...
    it('should round to 6 decimal places', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-4', 1, 1);
      // Very small costs
    ...
    it('should handle large token counts', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'gpt-4', 1000000, 500000);
  ...
  describe('get summary', () => {
    ...
    it('should break down cost by provider', () => {
      ...
      expect(summary.costByProvider['openai']).not.toEqual(summary.costByProvider['anthropic']);
    });
    ...
    it('should break down cost by model', () => {
      ...
      expect(summary.costByModel['gpt-4']).toBeGreaterThan(summary.costByModel['gpt-3.5-turbo']);
    });
  });
  ...
  describe('custom pricing', () => {
    it('should allow setting custom pricing', () => {
      ...
      const operation = tracker.trackOperation('op-1', 'custom', 'custom-model', 1000, 1000);
    ...
    it('should override existing provider pricing', () => {
      ...
      const operation = tracker.trackOperation('op-1', 'openai', 'custom-gpt-4', 1000, 1000);
  ...
  describe('edge cases', () => {
    ...
    it('should handle unknown model gracefully', () => {
      const operation = tracker.trackOperation('op-1', 'openai', 'unknown-model', 1000, 500);
      expect(operation.totalCost).toBe(0);
    });
    ...
    it('should preserve operation history independently', () => {
      ...
      expect(tracker1.getSummary().totalCost).toBeGreaterThan(tracker2.getSummary().totalCost);
    });
  });
});
