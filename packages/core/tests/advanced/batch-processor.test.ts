...
import {
  ...
  type BatchJob,
  type BatchOptions,
} from '../../src/advanced/batch-processor';
...
describe('BatchProcessor', () => {
  ...
  describe('cancel job', () => {
    it('should cancel processing job', async () => {
      ...
      const jobPromise = processor.submitBatch(
        ...
        async (item) => new Promise((resolve) => setTimeout(() => resolve(item), 50))
      );
  ...
  describe('edge cases', () => {
    ...
    it('should handle items returning different types', async () => {
      const job = await processor.submitBatch([1, 'two', { three: 3 }], async (item) => item);
