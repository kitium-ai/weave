...
import { SchemaValidator, type SchemaType, type ObjectSchema, type ValidationResult } from '../../src/advanced/schema-validator'
      ...
      const schema: ObjectSchema = {
        password: {
          ...
          validate: (value) => {
            ...
            if (!pwd.match(/[A-Z]/)) return 'Must contain uppercase'
            if (!pwd.match(/[0-9]/)) return 'Must contain number'
            return true
          }
        }
      }
    ...
    it('should validate user registration data', () => {
      const schema: ObjectSchema = {
        ...
        password: {
          ...
          required: true,
          validate: (v) =>
            (v as string).length >= 8 ? true : 'Password too short'
        },
        name: { type: 'string', required: true }
      }
