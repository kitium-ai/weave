...
export function useProviderRouting(
  ...
  const emitRoutingEvent = useCallback((event: ProviderRoutingEvent) => {
    setLastEvent(event);
  ...
  useEffect(() => {
    (routerRef.current as any).__emitRoutingEvent = emitRoutingEvent;
  }, [emitRoutingEvent]);
...
export function useProviderNotifications(
  options: UseProviderNotificationsOptions
): void {
